...
class DownloadTask {
  ...
  /// Detect file extension from URL, preserving original binary format.
  /// Falls back to .ipa for iOS app packages.
  static String _extFromUrl(String url) {
    try {
      ...
      if (dot >= 0) {
        final ext = lastSeg.substring(dot).toLowerCase();
        if (['.ipa', '.apk', '.zip', '.pkg', '.deb'].contains(ext)) return ext;
      }
    } catch (_) {}
    return '.ipa'; // default for iOS app packages
  }
  ...
  Future<void> _download() async {
    try {
      ...
      final ext = _extFromUrl(app.downloadUrl);
      final path = '${dir.path}/${safe}_${app.id.hashCode.abs()}$ext';
      filePath = path;
