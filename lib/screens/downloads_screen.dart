...
class _DownloadsScreenState extends State<DownloadsScreen>
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const Spacer(),
                  if (dl.tasks.isNotEmpty)
                    _activeCountBadge(dl.activeCount, t),
                ],
              ),
            ),
            if (dl.tasks.isEmpty)
  ...
  Widget _activeCountBadge(int count, AppTheme t) {
...
class _DownloadCard extends StatelessWidget {
  ...
  // FIX: Save the binary file with correct MIME type
  Future<void> _saveToFiles(BuildContext context) async {
    final path = task.filePath!;
    final file = File(path);

    // Ensure the file exists and has actual bytes
    ...
    if (path.endsWith('.ipa')) {
      mimeType = 'application/octet-stream'; // IPA is a ZIP variant
    ...
    // Use XFile with explicit mimeType — share_plus sends binary correctly
    final xFile = XFile(path, mimeType: mimeType, name: path.split('/').last);
    await Share.shareXFiles(
      [xFile],
      subject: task.app.name,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isActive = status == DlStatus.downloading || isPaused;
...
class _StatusChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Text(label, style: t.sf(size: 12, color: color, weight: FontWeight.w500));
  }
}
